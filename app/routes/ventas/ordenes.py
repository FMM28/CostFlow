...
def _orden_o_403(id_orden):
    """Obtiene la orden y valida propiedad. Devuelve (orden, respuesta_redirect)."""
    orden = OrdenService.get_by_id(id_orden)
    if not orden:
        flash("Orden no encontrada", "error")
        return None, redirect(url_for("ventas.ordenes.index"))
    if orden.id_usuario != current_user.id_usuario:
        flash("Sin autorizacion para acceder a la orden", "error")
        return None, redirect(url_for("ventas.ordenes.index"))
    return orden, None


def _detalle_o_403(id_detalle):
    """Obtiene el detalle y valida que su orden pertenezca al usuario."""
    detalle = OrdenDetalleService.get_by_id(id_detalle)
    if not detalle:
        flash("Producto no encontrado", "error")
        return None, redirect(url_for("ventas.ordenes.index"))
    orden, resp = _orden_o_403(detalle.id_orden)
    if resp:
        return None, resp
    return detalle, None
...
def index():
    ...
    ordenes, total_pages, error = OrdenService.search_orders_by_user(
...
def create():
    ...
    return render_template("ventas/nueva_orden.html", today=today)
...
def create_post():
    try:
        ...
        return redirect(url_for("ventas.ordenes.detail", id_orden=orden.id_orden))
...
@role_required("ventas")
def detail(id_orden):
    orden, resp = _orden_o_403(id_orden)
    if resp:
        return resp
    ...
    return render_template(
        "ventas/detalle_orden.html", orden=orden, departamentos=departamentos
    )
...
def update(id_orden):
    ...
    return redirect(url_for("ventas.ordenes.detail", id_orden=id_orden))
...
def detail_show(id_detalle):
    detalle, resp = _detalle_o_403(id_detalle)
    if resp:
        return resp

    return render_template("ventas/detalle_show.html", detalle=detalle)
...
def detail_update(id_detalle):
    detalle_actual, resp = _detalle_o_403(id_detalle)
    if resp:
        return resp
...
def detail_proveedores(id_detalle):
    ...
    for error in errores:
        flash(
            f"Ocurrio un error al buscar proveedores: {error}, comunicate con un administrador",
            "error",
        )

    return render_template(
        "ventas/proveedores.html", productos=productos, id_detalle=id_detalle
    )
...
def detail_select_proveedor(id_detalle):
    ...
    if moneda and precio is not None:
        ...
        if error:
            flash(error, "error")
            return redirect(
                url_for("ventas.ordenes.detail_show", id_detalle=id_detalle)
            )
...
def preview_pdf(id_orden):
    ...
    return render_template(
        "ventas/pdf_preview.html",
        orden=orden,
        pdf_url=url_for(
            "ventas.ordenes.pdf_file",
            id_orden=id_orden,
        ),
    )
...
def agrupaciones_create(id_orden):
    ...
    return redirect(url_for("ventas.ordenes.agrupaciones", id_orden=id_orden))
