...
def create_user():
    return render_template(
        ...
        usuario=None,
        url_action=url_for("admin.create_user_post"),
        url_cancel=url_for("admin.users"),
    )
...
def create_user_post():
    ...
    if error:
        if url_firma and os.path.exists(filepath):
            os.remove(filepath)
        flash(error, "danger")
        return render_template("admin/form_user.html", usuario=None)
...
def edit_user(user_id):
    ...
    return render_template(
        "admin/form_user.html",
        usuario=usuario,
        url_action=url_for("admin.edit_user_post", user_id=usuario.id),
        url_cancel=url_for("admin.users"),
    )
...
def edit_user_post(user_id):
    ...
    print(request.files)
    print(request.files.get("firma_file"))
    ...
    if error:
        if firma_file and firma_file.filename != "" and os.path.exists(filepath):
            os.remove(filepath)
        flash(error, "danger")
        usuario = UserService.get_by_id(user_id)
