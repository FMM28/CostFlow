...
class OrdenDetalle(BaseModel):
    ...
    id_proveedor = db.Column(
        db.Integer, db.ForeignKey("proveedor.id_proveedor"), nullable=True
    )
    ...
    )  # Costo de envío por pieza
    ganancia_unitaria = db.Column(
        db.Numeric(10, 2), nullable=False, default=0
    )  # Margen por pieza
    ...
    agrupaciones = db.relationship(
        "AgrupacionDetalle", back_populates="detalle", cascade="all, delete-orphan"
    )
