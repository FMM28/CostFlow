...
def setup_logging(app) -> None:
    ...
    log_max_bytes = app.config.get("LOG_MAX_BYTES", 10 * 1024 * 1024)  # 10 MB por defecto
    ...
    # Log de inicio
    app.logger.info(f"Logging configurado: nivel={logging.getLevelName(log_level)}, archivo={log_file}")
...
class RequestFormatter(logging.Formatter):
    ...
    def format(self, record):
        ...
        if has_request_context():
            ...
            record.user_agent = request.user_agent.string[:100] if request.user_agent else 'Unknown'
        else:
            record.url = None
            record.method = None
